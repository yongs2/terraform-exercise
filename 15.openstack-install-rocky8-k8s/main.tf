...
resource "null_resource" "install_kuberernetes_master" {
  ...
  provisioner "local-exec" {
    command = <<EOF
      ...
      export KUBE_JOIN_CMD=$(ssh rocky@${openstack_compute_instance_v2.master[0].access_ip_v4} -o StrictHostKeyChecking=no -i ${var.ssh_key_file} "kubeadm token create --print-join-command") >> ./bin/03_kubeadm_join.sh
